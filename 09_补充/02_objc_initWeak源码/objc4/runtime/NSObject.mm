...
static id
/*
 location: __weak指针的地址，存储指针的地址
 newObj: 所引用的对象。即例子中的obj。
 objc_initWeak:调用该函数，并传以下参数：
 
 HaveOld=false[weak指针之前是否指向了一个弱引用]
 HaveNew=true[weak指针是否需要指向一个新的引用]
 CrashIfDeallocating=true[若果被弱引用的对象正在析构，此时再弱引用该对象是否应该crash]
 */
storeWeak(id *location, objc_object *newObj)
{
 ...
 retry:
    ...
    if (HaveNew) {
        //有新的引用，则从全局的SideTables中取出该obj对应的SideTable
    ...
    if (HaveNew) {
        ...
        if (newObj  &&  !newObj->isTaggedPointer()) {
            newObj->setWeaklyReferenced_nolock()/*更新newObj的isa的weakly_referenced bit标志位*/;
        }
