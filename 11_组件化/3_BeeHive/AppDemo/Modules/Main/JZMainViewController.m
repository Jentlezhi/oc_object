...
//    Protocol *protocol1 = NSProtocolFromString(@"JZDetailServiceProtocol");
//    Protocol *protocol2 = NSProtocolFromString(@"JZDetailServiceProtocol3");
//    if ([self conformsToProtocol:protocol1]) {
//        NSLog(@"**");
//    }
...
- (void)jumpAction {
//    id<JZDetailServiceProtocol> detail = [[BeeHive shareInstance] createService:@protocol(JZDetailServiceProtocol)];
//    detail.info = self.textField.text;
//    [self.navigationController pushViewController:detail animated:YES];
    
    //对于MainViewContorller,只知道JZDetailServiceProtocol这个协议，其他的一概不知道，解耦
    id detail = [[BeeHive shareInstance] createService:@protocol(JZDetailServiceProtocol)];
    UIViewController *vc = [detail featchDetailVCWithInfo:self.textField.text valueCallback:^(NSString *value) {
